...
<body>
    ...
    foreach ($folders as $folder) {
        // var_dump($file);
        if ($folder != "." && $folder != "..") {
            $date = date("Y-m-d", strtotime($folder));
            $today = date("Y-m-d");
            if ($date == $today) {
                ...
                if (explode(".", $file)[1] == "png" || explode(".", $file)[1] == "jpg" || explode(".", $file)[1] == "jpeg"   || explode(".", $file)[1] == "gif") {
                    echo "
                    <div class='dinamic-scene' id='scene-$c'>
                        <img src='$dir/$date/$file' style='width:100%;height:100%;object-fit:cover;'>
                    </div>
                    ";
                }
            }
    ...
    foreach ($files as $file) {
        if ($file != "." && $file != ".." && $files >= 3) {
            ...
            if (explode(".", $file)[1] == "png" || explode(".", $file)[1] == "jpg" || explode(".", $file)[1] == "jpeg" || explode(".", $file)[1] == "gif") {
                echo "
                <div class='dinamic-scene' id='scene-$c'>
                    <img src='$dir/$file' style='width:100%;height:100%;object-fit:cover;'>
                </div>
                ";
            }
            //If file type video
            if (explode(".", $file)[1] == "mp4" || explode(".", $file)[1] == "webm" || explode(".", $file)[1] == "ogg") {
                echo "
                <div class='dinamic-scene' id='scene-$c'>
                    <video src='$dir/$file' style='width:100%;height:100%;object-fit:cover;'></video>
                </div>
                ";
            }
        }
            ...
            $('.chevron-left').click(function() {
                ...
                for (let i = 0; i < sceneQuantity; i++) {
                    ...
                    if ($(element).is(':visible')) {
                        ...
                        if (i - 1 == -1) {
                            $(element).hide();
                            $($('.dinamic-scene')[sceneQuantity - 1]).show(0);
