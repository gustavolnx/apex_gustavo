    ...
    <div class="scene-3">
        ...
        <div class="graph">
            ...
            <div id="bars">
                ...
                <?php
                $objetivo = $info[1]['objetivo'];
                $objetivo = explode(',', $objetivo)[0];
                $objetivo = (int) round($objetivo / 1000, 0);
                // var_dump($objetivo);
                $index = 0;
                $contains = 0;
                $media = 0;
        ...
        <!-- <div id="goalmeter">
            ...
            $objetivo = str_replace('K', '', $objetivo);
            $media = str_replace('K', '', $media);
            $faltante = (int) ($objetivo - $media);
            // var_dump($objetivo, $media, $faltante);
            if ($faltante > 0) {
            ?>
                <div id="missing">
                    <p id="title">Balanço:</p>
                    <p id="value">-<?= $faltante ?>K
                    <div style="color:#ff0000;" class="unit">R$</div>
                    </p>
                </div>
            <?php
    ...
    <script>
        ...
        function graph() {
            ...
            if (run < 1) {
                ...
                var max = mid * 2;
                // console.log(mid, max);
                $('.left-indicator.max').text(max + 'K');
                $('.left-indicator.mid').text(mid + 'K');
                var low = 0;
                $('.left-indicator.low').text(low + 'K');
                ...
                function animateBars() {
                    ...
                    for (var i = 0; i < lenght - 1; i++) {
                        ...
                        //REMOVE K
                        //Change 'K' to 'R$' from $barTop and $bar text
                        barText = barText.replace('K', '');
                        $(barTop).children('.text').text(barText);
                        $(bar).children('.text').text(barText);
                        num = parseInt(barText);
                        var percent = Math.ceil((num / max) * 100);
                        var top = 100 - percent;
                        if (percent > 100) {
                            percent = 100;
                            $('.left-indicator.max').text(higher + 'K');
                        }
                        if (percent > 50) {
                            if (percent < 55) {
                                ...
                                $(bar).css('height', '0%');
                                $(bar).children('.text').text(barText + 'K');
                                ...
                                $(barTop).children('.text').text(barText + 'K');
                                $(bar).css('height', '0%');
                                //If bar is not :animated
                                if (!$(bar).is(':animated')) {
                                    $(bar).animate({
